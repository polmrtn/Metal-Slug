...
void BulletAnim::LoadTexture(int type) {
    ...
    {
        Image img = LoadImage("Graphics/bullets/bullet.png");
        bulletPlayerImg = LoadTextureFromImage(img);
    }
    else if (type == 2) {
        Image img = LoadImage("Graphics/explosions/grenade_enemy.png");
        spriteSheet = LoadTextureFromImage(img);
        SetTextureFilter(spriteSheet, TEXTURE_FILTER_POINT);
    }
    else if (type == 3) {
        Image img = LoadImage("Graphics/bullets/bulletrifle.png");
