...
void Player::SetCrouchHitbox() {
    hitboxWidth = 20.0f * SCALE;
    hitboxHeight = 25.0f * SCALE;           // Recortado por arriba
    hitboxOffsetX = 2.0f * SCALE;
    hitboxOffsetY = 23.0f * SCALE;          // Bajado para que los pies toquen el suelo
}
...
void Player::DrawCrouch() {
    ...
    if (anim.IsCrouchTransition()) {
        rowY = 18 * 34.0f;
        currentHeight = 68.0f;
        yOffset = -72.0f; //negativo hacia arriba positivo hacia abajo
    }
    else {
        rowY = 20 * 34.0f;
        currentHeight = 34.0f;
        yOffset = 75.0f; //negativo hacia arriba positivo hacia abajo
    }
...
Rectangle Player::GetHitBox() {
    ...
    if (!grounded && !crouching) {
        // Offsets específicos para salto
        float jumpOffsetX = 6.0f * SCALE;  // Ajusta este valor (positivo = derecha, negativo = izquierda)
        ...
        // Offsets normales para idle, correr, etc.
        if (dir == PlayerDirection::LEFT) {
